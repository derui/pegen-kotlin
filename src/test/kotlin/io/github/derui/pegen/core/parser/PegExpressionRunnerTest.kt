...
class PegExpressionRunnerTest {
    ...
    fun `parse expression`() {
        ...
        // Act
        val actual = PegExpressionRunner(PegExpression(listOf(seq), UUID.randomUUID())).run(source, context)
    ...
    fun `return first matched sequence`() {
        ...
        // Act
        val actual = PegExpressionRunner(PegExpression(listOf(seq, seq2), UUID.randomUUID())).run(source, context)
